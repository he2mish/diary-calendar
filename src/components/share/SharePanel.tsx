...
export default function SharePanel() {
  ...
  const handleInvite = async () => {
    ...
    setLoading(true);
    setError(null);
    const err = await inviteUser(username.trim().toLowerCase(), permission);
    if (err) setError(err);
    else setUsername('');
    setLoading(false);
  };
              ...
              {pendingInvites.map((invite) => (
                <div key={invite.id} className="flex items-center justify-between py-2 border-b border-gray-100">
                  <div>
                    ...
                    <p className="text-xs text-gray-400">@{invite.ownerUsername}</p>
                  </div>
            ...
            <div className="mb-4">
              ...
              {myShares.map((share) => (
                <div key={share.id} className="flex items-center justify-between py-2 border-b border-gray-100">
                  <div>
                    <p className="text-sm">@{share.sharedWithUsername}</p>
              ...
              {sharedWithMe.map((share) => (
                <div key={share.id} className="flex items-center justify-between py-2 border-b border-gray-100">
                  <div>
                    ...
                    <p className="text-xs text-gray-400">@{share.ownerUsername}</p>
                  </div>
